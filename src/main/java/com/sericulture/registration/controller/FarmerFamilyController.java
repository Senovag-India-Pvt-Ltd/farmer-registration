...
@RestController
@RequestMapping("/v1/farmer-family")
public class FarmerFamilyController {
    ...
    @Operation(summary = "Insert FarmerFamily Details", description = "Creates FarmerFamily Details in to DB")
    @ApiResponses(value = {
                    ...
                    content =
                            {
                                    ...
                                    @Schema(example = "{\"errorType\":\"VALIDATION\",\"message\":[{\"message\":\"FarmerFamily name should be more than 1 characters.\",\"label\":\"name\",\"locale\":null}]}"))
                            }),
            @ApiResponse(responseCode = "500", description = "Internal Server Error - Error occurred while processing the request.")
    })
    @PostMapping("/add")
    public ResponseEntity<?> addFarmerFamilyDetails(@Valid @RequestBody FarmerFamilyRequest farmerFamilyRequest){
        ResponseWrapper rw = ResponseWrapper.createWrapper(FarmerFamilyResponse.class);

        rw.setContent(farmerFamilyService.insertFarmerFamilyDetails(farmerFamilyRequest));
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "204", description = "No Content - inserted successfully",content =
                    {
                            @Content(mediaType = "application/json", schema =
                            @Schema(example = "{\"content\":{\"totalItems\":1,\"farmerFamily\":[{\"id\":1,\"farmerFamilyName\":\"\",\"relationShipId\":\"1\",\"farmerId\":\"1\"},{\"id\":2,\"farmerFamilyName\":\"FarmerFamilyName 1\",\"farmerId\":\"2\", \"relatioshipId\":\"2\"}],\"totalPages\":1,\"currentPage\":0},\"errorMessages\":[]}"))
                    }),
            @ApiResponse(responseCode = "400", description = "Bad Request - Has validation errors",
                    content =
                            {
    ...
    public ResponseEntity<?> getPaginatedList(
        ...
        rw.setContent(farmerFamilyService.getPaginatedFarmerFamilyDetails(PageRequest.of(pageNumber, size)));
        return ResponseEntity.ok(rw);
    }
    ...
    public ResponseEntity<?> deleteFarmerFamilyDetails(
            @PathVariable final Integer id
    ) {
        ResponseWrapper rw = ResponseWrapper.createWrapper(FarmerBankAccountResponse.class);

        rw.setContent(farmerFamilyService.deleteFarmerFamilyDetails(id));
        return ResponseEntity.ok(rw);
    }
    ...
    @PostMapping("/edit")
    public ResponseEntity<?> editFarmerFamilyDetails(
            @Valid @RequestBody final EditFarmerFamilyRequest editFarmerFamilyRequest
    ) {
        ResponseWrapper<FarmerFamilyResponse> rw = ResponseWrapper.createWrapper(FarmerFamilyResponse.class);
        rw.setContent(farmerFamilyService.updateFarmerFamilyDetails(editFarmerFamilyRequest));
        return ResponseEntity.ok(rw);
    }
